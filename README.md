...
Juggling Lab is written in Kotlin and uses standard Swing components, so it should run on any platform supported by Java 11 or later. The ``pom.xml`` file is used by [Maven](https://maven.apache.org/) to build the code, and scripts in ``/bin`` run the compiled jar.
