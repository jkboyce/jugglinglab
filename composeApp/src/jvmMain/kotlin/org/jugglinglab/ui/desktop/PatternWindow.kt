...
import org.jugglinglab.ui.common.LadderDiagramLayout
...
import org.jugglinglab.view.SimpleView
...
class PatternWindow(
    ...
    private fun doMenuCommand(action: MenuCommand) {
        when (action) {
            ...
            MenuCommand.VIEW_ZOOMIN -> view.onViewZoomChange.invoke(ZOOM_PER_STEP)
            MenuCommand.VIEW_ZOOMOUT -> view.onViewZoomChange.invoke(1f / ZOOM_PER_STEP)
    ...
    companion object {
        private const val ZOOM_PER_STEP: Float = 1.1f
        ...
        private val optimizerWrapper: OptimizerWrapper? by lazy {
            runCatching {
                val optimizerClass = Class.forName("org.jugglinglab.optimizer.Optimizer")
