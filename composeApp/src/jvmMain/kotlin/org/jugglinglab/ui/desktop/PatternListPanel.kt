...
import org.jugglinglab.jml.JmlNode
...
class PatternListPanel(
    ...
    private fun changeDisplayText(row: Int) {
        ...
        okButton!!.addActionListener { _: ActionEvent? ->
            val updatedRec = rec.copy(display = tf!!.getText())
            patternList.model[row] = updatedRec
            dialog!!.dispose()
        ...
        override fun importData(info: TransferSupport): Boolean {
            ...
            try {
                if (t.isDataFlavorSupported(PATTERN_FLAVOR)) {
                    ...
                    listModel.add(index, PatternRecord.create(rec))
    ...
    private inner class PatternListModel : AbstractListModel<PatternRecord>() {
        ...
        fun remove(element: PatternRecord): Boolean {
            val index = patternList.model.indexOfFirst { it === element }
            return if (index != -1) {
                remove(index)
                true
            } else false
