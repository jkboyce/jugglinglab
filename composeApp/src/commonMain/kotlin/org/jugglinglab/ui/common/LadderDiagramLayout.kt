...
class LadderDiagramLayout(
    ...
    var transitionRadius = (TRANSITION_RADIUS_DP * density).toInt()
    var positionRadius = (POSITION_RADIUS_DP * density).toInt()
    ...
    private fun calculateLayout() {
        ...
        if (jlIsMobile) {
            ...
            val separationPixels = buildList {
                for (j in 1..pattern.numberOfJugglers) {
                    for (h in listOf(JmlEvent.LEFT_HAND, JmlEvent.RIGHT_HAND)) {
                        val items = eventItems.filter { it.event.juggler == j && it.event.hand == h }
                        for (i in 1..items.lastIndex) {
                            val sep = timeToY(items[i].event.t) - timeToY(items[i - 1].event.t)
                            if (sep > 0) {
                                add(sep)
                            }
                        }
            ...
            // enforce into a certain range
            transitionRadius = transitionRadius.coerceIn(minTransitionRadius, borderTop - 3)
            positionRadius = positionRadius.coerceIn(minPositionRadius, borderTop - 3)
        }
        ...
        for (item in pathItems) {
            ...
            if (slot != -1) {
                ...
                if (item.type == LadderItem.TYPE_SELF) {
                    // circular arc for same-handed self throw
                    val dx = (item.xStart - item.xEnd).toDouble()
                    val dy = (item.yStart - item.yEnd).toDouble()
                    val a = 0.5 * sqrt(dx * dx + dy * dy)
                    val xt = 0.5 * (item.xStart + item.xEnd).toDouble()
                    val yt = 0.5 * (item.yStart + item.yEnd).toDouble()

                    // center of the ladder (pixels)
                    val center: Double = 0.5 * (rightX + leftX) + (item.endEvent.juggler - 1) * jugglerDeltaX
    ...
    companion object {
        ...
        const val SELFTHROW_WIDTH: Double = 0.8

        // Return preferred width of the overall panel, in logical pixels.

        fun getPreferredWidthDp(jugglers: Int, paths: Int, textMeasurer: TextMeasurer): Int {
            if (jugglers > MAX_JUGGLERS) {
                ...
                // allocate enough space for a "too many paths" message
                val text = jlGetStringResource(Res.string.gui_too_many_paths, MAX_PATHS)
                val textLayoutResult = textMeasurer.measure(
                    text = text,
                    style = TextStyle(color = Color.Black, fontSize = 13.sp)
                )
