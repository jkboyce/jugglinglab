...
class AnimationController(
    ...
    var currentLayout: AnimationLayout? by mutableStateOf(null)
        private set
    ...
    fun restartJuggle(
        ...
        if (pattern == null && prefs == null) state.update(selectedItemHashCode = 0)
        if (coldRestart) {
            state.update(
    ...
    fun handleRelease(cancel: Boolean = false) {
        mouseReleasedLogic(cancel)
    }
    ...
    fun handleZoom(zoomFactor: Float) {
        val newZoom = (state.zoom * zoomFactor).coerceIn(MIN_ZOOM, MAX_ZOOM)
        state.update(zoom = newZoom)
    }
    ...
    private fun mouseReleasedLogic(cancel: Boolean = false) {
        if (!cancel && !draggingCamera && !dragging) {
