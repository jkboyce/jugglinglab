...
class SquareProp : Prop() {
    ...
    private fun createImage(zoom: Double) {
        var squarePixelSize = (0.5 + zoom * diam).toInt()
        squarePixelSize = max(squarePixelSize, 1)

        // Create a square image of size squarePixelSize with a transparent background
        val newImage = ImageBitmap(squarePixelSize + 1, squarePixelSize + 1)
        val canvas = Canvas(newImage)
        val paint = Paint()

        if (highlight) {
            val highlightSquares = squarePixelSize / 1.2f // Number of concentric squares to draw.
            ...
            canvas.drawRect(
                rect = Rect(0f, 0f, squarePixelSize.toFloat(), squarePixelSize.toFloat()),
                paint = paint
            )

            // Draw the highlight as a series of concentric, lighter squares
            for (i in 0 until highlightSquares.toInt()) {
                // Progressively lighten the color towards white
                currentColor = Color(
                    red = min(currentColor.red + (1f / highlightSquares), 1f),
                    green = min(currentColor.green + (1f / highlightSquares), 1f),
                    blue = min(currentColor.blue + (1f / highlightSquares), 1f),
                ...
                val left = (i / 1.1f)
                val top = (i / 2.5f)
                val size = squarePixelSize - (i * 1.3f)
                canvas.drawRect(Rect(left, top, left + size, top + size), paint)
            }
        } else {
            paint.color = color
            canvas.drawRect(
        ...
        image = newImage
        size = IntSize(squarePixelSize, squarePixelSize)
        center = IntSize(squarePixelSize / 2, squarePixelSize / 2)
        grip = IntSize(squarePixelSize / 2, squarePixelSize / 2)
        lastzoom = zoom
    }
