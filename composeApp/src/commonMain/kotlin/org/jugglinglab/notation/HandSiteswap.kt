...
private fun getNumericValue(c: Char): Int {
    if (c in '0'..'9') return c - '0'
    if (c in 'a'..'z') return c - 'a' + 10
    if (c in 'A'..'Z') return c - 'A' + 10
    return -1
}
...
object HandSiteswap {
        ...
        for (i in 0..<ss.length) {
            ...
            if (muxThrow) {
                if (c.toString().matches("[0-9,a-z]".toRegex())) {
                    ...
                    val message =
                        jlGetStringResource(Res.string.error_hss_object_syntax_error_at_pos, i + 1)
                    throw JuggleExceptionUser(message)
                }
            } else {
                if (c.toString().matches("[0-9,a-z]".toRegex())) {
    ...
    private fun ossPermTest(os: ArrayList<ArrayList<Char>>, op: Int) {
        ...
        for (i in 0..<op) {
            ...
            for (j in os[i].indices) {
                modulo = (getNumericValue(os[i][j]) + i) % op
                mods[i].add(j, modulo)
                cmp[modulo]++
            }
        }
    ...
    private fun hssPermTest(hs: ArrayList<Char>, hp: Int): Int {
        ...
        for (i in 0..<hp) {
            modulo = (getNumericValue(hs[i]) + i) % hp
            mods[i] = modulo
            cmp[modulo]++
        }
        ...
        for (i in 0..<hp) {
            if (!touched[i]) {
                orb[i] = getNumericValue(hs[i])
                touched[i] = true
                var j = mods[i]
                while (j != i) {
                    orb[i] += getNumericValue(hs[j])
                    touched[j] = true
                    j = mods[j]
                }
            }
    ...
    private fun convertNotation(
        ...
        for (i in 0..<patPer) {
            if (hs[i] == '0') {
                ...
                if (!assignDone[i]) {
                    ...
                    ah[i] = currHand
                    assignDone[i] = true
                    var next = (i + getNumericValue(hs[i])) % patPer
                    while (next != i) {
                        ah[next] = currHand
                        assignDone[next] = true
                        next = (next + getNumericValue(hs[next])) % patPer
                    }
                }
        ...
        for (i in 0..<patPer) {
            for (j in os[i].indices) {
                curThrow = getNumericValue(os[i][j])
        ...
        for (i in 0..<patPer) {
            ...
            for (j in os[i].indices) {
                iph[i].add(j, null)
                throwVal = getNumericValue(os[i][j])
                ...
                if (sourceJug != targetJug) {
                    ...
                    if (throwVal == getNumericValue(hs[i])) {
                        if (iph[i][j] !== "x") {
