...
public class JMLPattern {
  ...
  public void setTitle(String t) {
    ...
    title = ((t != null && !t.isBlank()) ? t.strip() : null);
  ...
  public void setInfo(String info_string) {
    if (info_string != null && !info_string.isBlank()) {
  ...
  public void addEvent(JMLEvent ev) {
    ...
    while (true) {
      ...
      if (combine_events) {
        ...
        for (JMLTransition tr_current : current.getTransitions()) {
  ...
  public JMLEvent getEventImageInLoop(JMLEvent ev) {
    JMLEvent current = eventlist;
    while (current != null) {
      if (current.t >= getLoopStartTime()
          && current.t < getLoopEndTime()
          && current.getJuggler() == ev.getJuggler()
          && current.getHand() == ev.getHand()
  ...
  protected void printEventList() {
    ...
    while (current != null) {
      if (current.isMaster()) {
        ...
        System.out.println("  Slave event; master at t=" + current.getMaster().t);
      }
    ...
    try {
      ...
      for (int path = 1; path <= getNumberOfPaths(); ++path) {
        for (PathLink pl : getPathLinks().get(path - 1)) {
          ...
          for (JMLTransition tr : start.getTransitions()) {
            if (tr.path == path) {
              start_tr = tr;
              break;
            }
          ...
          for (JMLTransition tr : end.getTransitions()) {
            if (tr.path == path) {
              end_tr = tr;
              break;
            }
  ...
  public void streamlinePatternWithWindow(double twindow)
    ...
    while (ev != null) {
      ...
      boolean inside_window = (prev != null && (ev.t - prev.t) < twindow);
  ...
  public void buildEventList() throws JuggleExceptionInternal, JuggleExceptionUser {
    ...
    do {
      ...
      if (maxtime < getLoopStartTime()) {
        ...
        for (JMLTransition tr : maxevent.getTransitions()) {
          int path = tr.path - 1;
          ...
          switch (tr.getType()) {
            ...
            case JMLTransition.TRANS_HOLDING:
              if (!hasVDPathJMLTransition[path]) {
    ...
    do {
      ...
      if (mintime > getLoopEndTime()) {
        ...
        for (JMLTransition tr : minevent.getTransitions()) {
          int path = tr.path - 1;
  ...
  public void findMasterEvents() throws JuggleExceptionInternal, JuggleExceptionUser {
    ...
    while (ev != null) {
      if (ev.isMaster()) {
        ...
        if (ev.t >= getLoopStartTime() && ev.t < tmaster) {
          tmaster = ev.t;
        }
        ...
        while (ev2 != null) {
          if (ev2.getMaster() == ev) {
            if (ev2.t >= getLoopStartTime() && ev2.t < tmaster) {
              newmaster = ev2;
              tmaster = ev2.t;
            }
          }
        ...
        if (newmaster != ev) {
          ...
          while (ev2 != null) {
            if (ev2.getMaster() == ev) {
              ev2.setMasterEvent(newmaster);
            }
            ev2 = ev2.getNext();
          }
          newmaster.setMasterEvent(null);
          ev.setMasterEvent(newmaster);
        }
      }
      ev = ev.getNext();
    }
  ...
  public void gotoGlobalCoordinates() {
    ...
    while (ev != null) {
      ...
      Coordinate gc = convertLocalToGlobal(lc, ev.getJuggler(), ev.t);
  ...
  protected void buildLinkLists() throws JuggleExceptionUser, JuggleExceptionInternal {
    pathlinks = new ArrayList<>(getNumberOfPaths());

    for (int i = 0; i < getNumberOfPaths(); i++) {
      // build the PathLink list for the ith path
      pathlinks.add(new ArrayList<>());
      JMLEvent ev = eventlist;
    ...
    for (int i = 0; i < getNumberOfJugglers(); i++) {
      ...
      for (int j = 0; j < 2; j++) {
        ...
        while (true) {
          ...
          while (ev.getJuggler() != (i + 1) || ev.getHand() != handnum) {
            ev = ev.getNext();
            if (ev == null) {
              break done2;
            }
      ...
      for (int h = 0; h < 2; h++) {
        ...
        if (hasVDHandJMLTransition[j][h]) {
          ...
          while (k < handlinks.get(j).get(h).size()) {
            ...
            if (hl.getEndEvent().t > getLoopStartTime()) {
              break;
            }
            ++k;
          }
  ...
  public void getPathCoordinate(int path, double time, Coordinate newPosition)
    ...
    for (PathLink pl : pathlinks.get(path - 1)) {
      if (time >= pl.getStartEvent().t && time <= pl.getEndEvent().t) {
        if (pl.isInHand()) {
  ...
  public void getHandCoordinate(int juggler, int hand, double time, Coordinate newPosition)
    ...
    for (HandLink hl : handlinks.get(juggler - 1).get(handindex)) {
      if (time >= hl.getStartEvent().t && time < hl.getEndEvent().t) {
  ...
  public double getPathCatchVolume(int path, double time1, double time2) {
    ...
    for (i = 0; i < pathlinks.get(path - 1).size(); i++) {
      ...
      if (time1 >= pl1.getStartEvent().t && time1 <= pl1.getEndEvent().t) break;
    }
    ...
    while (true) {
      ...
      if (time2 >= pl2.getStartEvent().t && time2 <= pl2.getEndEvent().t) break;
  ...
  public double getPathBounceVolume(int path, double time1, double time2) {
    ...
    for (i = 0; i < pathlinks.get(path - 1).size(); i++) {
      ...
      if (time1 >= pl.getStartEvent().t && time1 <= pl.getEndEvent().t) {
        break;
      }
    }
    ...
    while (true) {
      pl = pathlinks.get(path - 1).get(i);
      Path p = pl.getPath();
      if (p instanceof BouncePath bp) {
        double vol = bp.getBounceVolume(time1, time2);
        if (vol > 0) {
          return vol;
        }
      }
      if (time2 >= pl.getStartEvent().t && time2 <= pl.getEndEvent().t) {
        break;
      }
    ...
    if (type.equalsIgnoreCase("jml")) {
      ...
      try {
        if (jugglerstring != null) {
          setNumberOfJugglers(Integer.parseInt(jugglerstring));
        } else {
          setNumberOfJugglers(1);
        }
        setNumberOfPaths(Integer.parseInt(pathstring));
      } catch (Exception ex) {
  ...
  public void writeJML(Writer wr, boolean write_title, boolean write_info) throws IOException {
    ...
    for (JMLSymmetry symmetry : symmetries) {
