...
public class Mutator {
  ...
  protected JMLPattern mutateEventTime(JMLPattern pat)
    ...
    double tmin =
        (ev_prev == null
            ...
            : Math.max(pat.getLoopStartTime(), ev_prev.getT()) + mutationMinEventDeltaSec);
  ...
  protected JMLPattern mutateAddEvent(JMLPattern pat)
    ...
    for (int path = 1; path <= pat.getNumberOfPaths(); path++) {
      if (pat.isHandHoldingPath(juggler, hand, t, path)) {
        JMLTransition trans = new JMLTransition(JMLTransition.TRANS_HOLDING, path, null, null);
  ...
  protected JPanel makeControlPanel() {
    ...
    gb.setConstraints(
        lab, make_constraints(GridBagConstraints.LINE_START, 0, 0, new Insets(0, 0, 10, 0)));
    controls.add(lab);

    this.cb = new JCheckBox[5];

    this.cb[0] = new JCheckBox(guistrings.getString("Mutator_type1"), true);
    gb.setConstraints(cb[0], make_constraints(GridBagConstraints.LINE_START, 0, 1, null));
    controls.add(cb[0]);

    this.cb[1] = new JCheckBox(guistrings.getString("Mutator_type2"), true);
    gb.setConstraints(cb[1], make_constraints(GridBagConstraints.LINE_START, 0, 2, null));
    controls.add(cb[1]);

    this.cb[2] = new JCheckBox(guistrings.getString("Mutator_type3"), true);
    gb.setConstraints(cb[2], make_constraints(GridBagConstraints.LINE_START, 0, 3, null));
    controls.add(cb[2]);

    this.cb[3] = new JCheckBox(guistrings.getString("Mutator_type4"), true);
    gb.setConstraints(cb[3], make_constraints(GridBagConstraints.LINE_START, 0, 4, null));
    controls.add(cb[3]);

    this.cb[4] = new JCheckBox(guistrings.getString("Mutator_type5"), true);
    gb.setConstraints(cb[4], make_constraints(GridBagConstraints.LINE_START, 0, 5, null));
    controls.add(cb[4]);

    lab = new JLabel(guistrings.getString("Mutator_header2"));
    gb.setConstraints(
        lab, make_constraints(GridBagConstraints.LINE_START, 0, 6, new Insets(20, 0, 10, 0)));
    controls.add(lab);

    this.slider_rate = new JSlider(SwingConstants.HORIZONTAL, 0, 6, 3);
    GridBagConstraints gbc = make_constraints(GridBagConstraints.LINE_START, 0, 7, null);
