...
public class MHNBody {
  ...
  public MHNBody(String str) throws JuggleExceptionUser, JuggleExceptionInternal {
    ...
    for (int pass = 0; pass < 4; pass++) {
      ...
      for (int pos = 0; pos < str.length(); ) {
        ...
        if (ch == '-') {
          if (pass == 3) bodypath[juggler][beat][coordnum] = null;
          coordnum++;
          pos++;
          continue;
        }
  ...
  public JMLPosition getPosition(int juggler, int pos, int index) {
    if (pos >= getPeriod(juggler) || index >= getNumberOfPositions(juggler, pos)) return null;
    int j = (juggler - 1) % jugglers;
    if (bodypath[j][pos][index] == null) return null;
