...
public abstract class MHNPattern extends Pattern {
      ...
      for (int j = 0; j < getNumberOfJugglers(); j++) {
        for (int h = 0; h < 2; h++) {
          ...
          for (int slot = 0; slot < getMaxOccupancy(); slot++) {
            ...
            if (onethrown) sst2.throwtime = ((double) k - BEATS_ONE_THROW_EARLY) / bps;
            else sst2.throwtime = (double) k / bps;
          ...
          if (onecaught) {
            firstcatchtime =
                Math.max(
                    firstcatchtime,
                    ((double) (k - 1) - BEATS_ONE_THROW_EARLY + BEATS_AIRTIME_MIN) / bps);
          }

          // Constraint #3: Ensure we have enough time between catch
          // and subsequent throw
          firstcatchtime = Math.min(firstcatchtime, sst.throwtime - BEATS_CATCH_THROW_MIN / bps);
          ...
          for (int slot = 0; slot < getMaxOccupancy(); slot++) {
            ...
            if (num_catches > 1) {
              catchtime +=
                  ((double) sst2.catchnum / (double) (num_catches - 1)) * (squeezebeats / bps);
            }
            ...
            catchtime = Math.min(catchtime, sst.throwtime - BEATS_CATCH_THROW_MIN / bps);
  ...
  protected void addPrimaryEventsToJML(
      JMLPattern pat, boolean[][] handtouched, boolean[] pathtouched)
      throws JuggleExceptionUser, JuggleExceptionInternal {
    for (int j = 0; j < getNumberOfJugglers(); j++) {
      for (int h = 0; h < 2; h++) {
        ...
        for (int h = 0; h < 2; h++) {
          ...
          for (int slot = 0; slot < getMaxOccupancy(); slot++) {
            ...
            if (sst2.mod.charAt(0) != 'H') {
              ...
              ev.addTransition(
                  new JMLTransition(JMLTransition.TRANS_THROW, sst2.pathnum, type, mod));
  ...
  protected void addEventsForGapsToJML(JMLPattern pat)
      throws JuggleExceptionUser, JuggleExceptionInternal {
    for (int h = 0; h < 2; h++) {
