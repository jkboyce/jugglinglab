...
public class MHNThrow {
  ...
  public boolean isHold() {
    if ((targetindex - index) > 2 || hand != targethand || juggler != targetjuggler) return false;

    if (mod != null && mod.indexOf('T') != -1) return false;
  ...
  public static int compareThrows(MHNThrow mhnt1, MHNThrow mhnt2) {
    ...
    if (beats1 > beats2) return 1;
    else if (beats1 < beats2) return -1;
    ...
    // passes > self-throws
    if (is_pass1 && !is_pass2) return 1;
    else if (!is_pass1 && is_pass2) return -1;

    // for two passes, lower target juggler number wins
    if (is_pass1) {
      if (mhnt1.targetjuggler < mhnt2.targetjuggler) return 1;
      else if (mhnt1.targetjuggler > mhnt2.targetjuggler) return -1;
    }

    // crossed throws beat non-crossed throws
    if (is_cross1 && !is_cross2) return 1;
    else if (!is_cross1 && is_cross2) return -1;
    ...
    if (has_mod1 && !has_mod2) return 1; // throw with modifier beats one without
    else if (!has_mod1 && has_mod2) return -1;
    else if (has_mod1 && has_mod2) {
      int c = mhnt1.mod.compareTo(mhnt2.mod);

      if (c < 0) return 1; // mhnt1.mod lexicographically precedes mhnt2.mod
