...
class BouncePath : Path() {
  ...
  private fun solveBounceEquation(
    ...
    coef[2] =
      ((6 * gt * gt + 2 * kk * kk * u + 2 * f2 * f2 * c
          ) - 2 * f2 * c - 2 * f2 * gt * gt - 2 * kk * gt * gt - 2 * kk * u - 2 * kk * f2 * c - 2 * kk * f2 * u)
    coef[1] = -4 * gt * gt * gt + 4 * f2 * gt * c + 4 * kk * gt * u
    coef[0] =
      ((gt * gt * gt * gt + kk * kk * u * u + f2 * f2 * c * c
          ) - 2 * gt * gt * f2 * c - 2 * kk * gt * gt * u - 2 * kk * f2 * u * c)
    ...
    */
    ...
    private fun findRealRootsPolynomial(
      ...
      if (pinfpositive != extremumpositive[numextrema - 1]) {
        ...
        val endpoint2: Double = bracketOpenInterval(coef, degree, extremum[numextrema - 1], true)
